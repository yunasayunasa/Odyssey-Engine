...
export default class PreloadScene extends Phaser.Scene {
    constructor() {
        super('PreloadScene'); // main.jsでactive:trueが設定されるため、ここでは不要
    ...
    preload() {
        ...
        // ここで asset_define.json をロードし、その完了を待ってから他のアセットをキューに追加する
        // という二段階ロードがPhaserの標準では少し複雑なので、
        // 今回は asset_define.json は create() で読み取る前提で、
        // main.jsのconfig.sceneに PreloadScene が active:true であることと、
        // asset_define.json が Phaser の Load.BasePlugin でロード可能であることを前提とします。
        // （つまり、PreloadSceneのpreload()が呼ばれる前にasset_define.jsonがキャッシュされているか、
        // あるいはcreate()で手動でload.jsonを呼び出し、その後にload.start()を呼ぶ二段ロードにする）

        // まずは asset_define.json 自体をロードキューに追加
        this.load.json('asset_define', 'assets/asset_define.json');
        this.load.script('webfont', 'https://ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js');

        // ★★★ 修正箇所: preload()内でasset_define.jsonのロード完了を待つロジックを追加 ★★★
        // これは二段階ロードの典型的なパターンです。
        // preload()内でこのon('complete')が呼ばれるのは、preload()で追加されたアセットが全てロードされた後です。
        // つまり、asset_define.json と webfont.js がロードされた後、このコールバックが呼ばれます。
        this.load.once('complete', () => {
            console.log("PreloadScene: preload段階のアセットロード完了。asset_define.jsonを読み込みます。");
            const assetDefine = this.cache.json.get('asset_define');

            // asset_define.json の内容に基づいて、残りのアセットをロードキューに追加
            ...
            // ★★★ preload() の中で、最終的なロードを開始します。
            // これにより、create()は全てのアセットがロードされた後に実行されることが保証されます。
            this.load.start(); // ここでロードを開始
        }, this); // コールバックのスコープをPreloadSceneインスタンスにする

        // ★★★ 修正箇所: ここではロードを開始しない。上のload.once('complete')で開始するため。 ★★★
        // this.load.start(); // <<< この行は削除またはコメントアウト！
    }

    create() {
        console.log("PreloadScene: create 開始。全アセットがロード済みです。");
        // create() が呼ばれる時点では、全てのアセットはロード済みであるため、
        // ここではロードキューへの追加は行いません。
    ...
    stop() {
        ...
        // ロード画面UIを破棄 (これらはpreload()で作成されたオブジェクト)
