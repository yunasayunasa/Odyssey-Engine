// src/scenes/NovelOverlayScene.js (最終版 - 全ての改善を統合)

const Container = Phaser.GameObjects.Container; // MessageWindowがContainerを継承しているので必要
...
export default class NovelOverlayScene extends Phaser.Scene {
    constructor() {
        ...
        this.stateManager = null; // このStateManagerはOverlayScene専用
        this.messageWindow = null; 
        this.configManager = null; 
        this.layer = { character: null, cg: null, message: null }; // backgroundは通常持たない
        this.charaDefs = null; 
        this.characters = {}; // このシーンのキャラクター管理用

        this.inputBlocker = null; 
        this.choiceButtons = []; // 選択肢ボタンもOverlayScene専用
        this.pendingChoices = []; // 選択肢データ

        this.inputPointerDownListener = null; // input.on('pointerdown') リスナー用
        // this.eventEmitted = false; // 必要に応じて追加するが、overlay_endはタグハンドラで制御
    } 
    ...
    init(data) {
        ...
        // this.eventEmitted = false; // init時にリセットする場合はここに
    }
    ...
    create() {
        ...
        this.cameras.main.setBackgroundColor('rgba(0,0,0,0)'); // 背景を透明に設定
        
        // --- レイヤー生成 ---
        this.layer.character = this.add.container(0, 0).setDepth(10); // キャラクター
        this.layer.cg = this.add.container(0, 0).setDepth(5);       // 背景CG (もしあれば)
        this.layer.message = this.add.container(0, 0).setDepth(20);   // メッセージウィンドウ、選択肢ボタン

        // 全画面を覆う、透明で見えない入力ブロッカーを作成
        this.inputBlocker = this.add.rectangle(this.scale.width / 2, this.scale.height / 2, this.scale.width, this.scale.height)
            .setInteractive()
            .setVisible(false) // 初期状態は非表示
            .setDepth(100); // UIやメッセージウィンドウより手前に来るように高いDepthを設定
        
        // inputBlockerがクリックイベントを消費するように設定 (イベント伝播停止)
        this.inputBlocker.on('pointerdown', (pointer, localX, localY, event) => {
            event.stopPropagation();
        });


        this.configManager = this.sys.registry.get('configManager');
        this.stateManager = new StateManager(); // オーバーレイ専用のStateManagerインスタンス
        ...
        this.inputPointerDownListener = this.input.on('pointerdown', () => { 
    ...
    stop() {
        super.stop();
        console.log("NovelOverlayScene: stop されました。UI要素とマネージャーを破棄します。");
        ...
        // 各レイヤーのコンテナ内のPhaserオブジェクトを全て破棄
        if (this.layer.character) {
            this.layer.character.removeAll(true); 
            this.layer.character.destroy(); 
            this.layer.character = null;
        }
        if (this.layer.cg) { 
            this.layer.cg.removeAll(true);
            this.layer.cg.destroy();
            this.layer.cg = null;
        }
        if (this.layer.message) { 
            this.layer.message.removeAll(true);
            this.layer.message.destroy();
            this.layer.message = null;
        }
        
        // MessageWindow自身がPhaserオブジェクトなら破棄
        ...
        if (this.inputBlocker) {
            this.inputBlocker.off('pointerdown'); 
            this.inputBlocker.destroy(); 
            this.inputBlocker = null;
        }
        
        // 選択肢ボタンを破棄 (displayChoiceButtonsで生成される)
        this.clearChoiceButtons(); 

        // Managerクラスのインスタンスをnullにする (Phaserオブジェクトではないが参照をクリア)
        this.scenarioManager = null;
        this.soundManager = null;
